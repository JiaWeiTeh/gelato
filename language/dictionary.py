...
def build_prompt(selected_language):
    """Build the prompt dictionary for the given language code."""
    result = {}

    if selected_language == 'en':
        for key in prompt_raw:
            result[key] = key
        ...
        for key in prompt_raw:
            ...
            result[key] = meow_length + ['?', '.', '!', '.', '??'][randrange(5)]
    else:
        for key, val in prompt_raw.items():
            try:
                result[key] = val[selected_language]
            except KeyError:
                result[key] = key
...
def _init_prompt():
    """Initialize the module-level prompt dict from settings.

    Deferred import of settings to avoid circular imports and keep
    module-level side effects minimal.
    """
    from settings import settings
    config = settings.get_param()
    return build_prompt(config.general.def_language)


# Module-level prompt dict — initialized once on first import.
prompt = _init_prompt()
